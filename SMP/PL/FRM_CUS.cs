...
namespace SMP.Present_Lare
{
    ...
    gridControl1.DataSource = dbContext.TB_Cus.Local.ToBindingList();
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
         ...
         try
         {
            ...
            if(rs==DialogResult.Yes)
            {
                    ...
                    db.SaveChanges();
                    toast.txt_capation.Text = "تم حذف عميل";
                    toast.Show();
                    Update_data();
            }
         }
        ...
        private void btn_edit_Click(object sender, EventArgs e)
        {
            ...
            id = Convert.ToInt32(tileView1.GetFocusedRowCellValue("ID"));//
            tb_cus = db.TB_Cus.Where(x => x.ID == id).FirstOrDefault();//
            frm_add.edit_name.Text = tb_cus.Supp_Name.ToString();
            frm_add.edt_phone.Text = tb_cus.Supp_Phone.ToString();
            frm_add.edt_email.Text = tb_cus.Supp_Email.ToString();

            methods.by = tb_cus.Supp_Image;
